-- IMAGINA O SEGUINTE CENÁRIO: PRECISAMOS ATUALIZAR OS VALORES QUE ESTÃO ZERADOS EM UMA DETERMINADA TABELA
...
C.NOME AS CLIENTE,
A.VALOR_TOTAL AS TOTAL_VENDA,
0 AS TOTAL_ACUMULADO_POR_VENDA
FROM PEDIDOS A(NOLOCK)
INNER JOIN PRODUTOS B(NOLOCK) ON (B.ID_PRODUTO=A.ID_PRODUTO)
INNER JOIN CLIENTES C(NOLOCK) ON (C.ID_CLIENTE=A.ID_CLIENTE)
ORDER BY A.ID_CLIENTE
...
--VAMOS CRIAR UM CURSOR PARA ATUALIZAR OS VALORES DE TOTAL ACUMULADO POR VENDA
...
INNER JOIN PRODUTOS B(NOLOCK) ON (B.ID_PRODUTO=A.ID_PRODUTO)
INNER JOIN CLIENTES C(NOLOCK) ON (C.ID_CLIENTE=A.ID_CLIENTE)
ORDER BY A.ID_CLIENTE, A.ID_PEDIDO
...
WHILE @@FETCH_STATUS = 0 
	...
	BEGIN
		...
		UPDATE PERFORMANCE_CLIENTE SET TOTAL_ACUMULADO_POR_VENDA = @TOTAL_ACUMULADO WHERE ID_CLIENTE = @ID_CLIENTE AND ID_PEDIDO = @ID_PEDIDO		
...
SELECT * FROM PERFORMANCE_CLIENTE ORDER BY ID_CLIENTE, ID_PEDIDO
